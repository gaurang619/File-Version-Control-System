...
void serviceClient(){
	...
	while(1){
		...
		n=read(clientSD, command, 100);
		
		printf("\nGot command from client: %s \n", command);
		
		if(strcmp(command, "quit") == 0)
		{
			printf("Server child is quiting...\n");
			close(clientSD);
			exit(0);
		}
		else{
			...
			if (strcmp(operation, "get") == 0){
				...
				if(ack == 1){	//means file is present 
					...
					printf("Tranfering content of %s file from server to client.\n ", fileName);
				...
				if(ack == 1){	//File exists on client side
					...
					char ch;
					printf("Receiving data from client for %s file.\n", fileName);
					...
					printf("Not recieved any data for %s file from client\n", fileName);	
					
			}
			else{
				printf("Invalid command entered by client\n");
			}
			//change working directory to  the parent directory.
				chdir("..");
				printf("\nWaiting for next command from client...\n");
		}
